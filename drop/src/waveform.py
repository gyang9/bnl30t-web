...
class Waveform():
        ...
        if self.cfg.daisy_chain:
            for ch in self.ch_names:
                if '_b1' in ch:
                    ...
                    return None
                if '_b4' in ch:
                    n_samp_b4 = len(self.amp_mV[ch])
                    pre_trg_frac = 1.0-self.cfg.post_trigger
                    self.trg_pos = int(n_samp_b4*pre_trg_frac)
                    self.trg_time_ns = self.trg_pos*(SAMPLE_TO_NS)
        else:
    ...
    def subtract_flat_baseline(self):
        ...
        adc_to_mV_b4 = self.cfg.dgtz_dynamic_range_mV/(2**12-1)
        for ch in self.ch_names:
            val = self.raw_data[ch]
            if 'adc_b4_ch0' in ch and self.cfg.debug:
            ...
            if 'b4' in ch:
                self.amp_mV[ch] = amp*adc_to_mV_b4
            else:
    ...
    def define_time_axis(self):
        ...
        #    self.n_samp_b4 = len(self.amp_pe['adc_b4_ch1'])
        #else:
        #    self.n_samp_b4 = 0
