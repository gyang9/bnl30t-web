...
const float B1_TIME_CONVERSION = 2.0;
const float B4_TIME_CONVERSION = 16.0;
...
void compare_waveforms() {
    gSystem->mkdir("waveform_comparison_b4ch9_triggered", kTRUE);
    // File 1
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (trigger) {
            ...
            g1->SetTitle((std::string("Event ") + std::to_string(event_id1) + " (b4ch9 triggered > 1V) from File 1;Time (ns);mV").c_str());
            for(int j=0; j<240; j++) g1->SetPoint(j, j * B4_TIME_CONVERSION, -(waveform1[j] - baseline1) * ADC_TO_MV_B4);
            g1->SetLineColor(kBlue);
            g1->Draw("AL");
            c->SaveAs((std::string("test2_waveform_comparison_b4ch9_triggered/event_") + std::to_string(event_id1) + "_file1.png").c_str());
            delete c;
            delete g1;
        }
    }
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (trigger) {
            ...
            g2->SetTitle((std::string("Event ") + std::to_string(event_id2) + " (b4ch9 triggered > 1V) from File 2;Time (ns);mV").c_str());
            for(int j=0; j<240; j++) g2->SetPoint(j, j * B4_TIME_CONVERSION, -(waveform2[j] - baseline2) * ADC_TO_MV_B4);
            g2->SetLineColor(kRed);
            g2->Draw("AL");
            c->SaveAs((std::string("test2_waveform_comparison_b4ch9_triggered/event_") + std::to_string(event_id2) + "_file2.png").c_str());
            delete c;
            delete g2;
        }
    }
...
void plot_b1ch1_triggered_on_b4ch9_bu(const std::vector<std::string>& filenames) {
    gSystem->mkdir("b1ch1_waveforms_b4ch9_triggered", kTRUE);
    ...
    for (const auto& filename : filenames) {
        ...
        const char* trigger_branch_name = "adc_b4_ch9";
        const char* plot_branch_name = "adc_b1_ch1";
        t->SetBranchAddress(trigger_branch_name, trigger_waveform);
        t->SetBranchAddress(plot_branch_name, plot_waveform);
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (trigger) {
                ...
                mg->SetTitle((std::string("Event ") + std::to_string(event_id) + " from file " + filename_suffix + ";Time (ns);mV").c_str());

                float plot_baseline = get_baseline(plot_waveform, 1920);
                TGraph *g_plot = new TGraph(1920);
                for(int j=0; j<1920; j++) g_plot->SetPoint(j, j * B1_TIME_CONVERSION, -(plot_waveform[j] - plot_baseline) * ADC_TO_MV_B1);
                g_plot->SetLineColor(kBlue);
                g_plot->SetLineWidth(2);
                mg->Add(g_plot);

                TGraph *g_trigger = new TGraph(240);
                for(int j=0; j<240; j++) g_trigger->SetPoint(j, j * B4_TIME_CONVERSION, -(trigger_waveform[j] - trigger_baseline) * ADC_TO_MV_B4);
                g_trigger->SetLineColor(kRed);
                g_trigger->SetLineWidth(2);
                mg->Add(g_trigger);

                mg->Draw("AL");

                TLegend *leg = new TLegend(0.7, 0.8, 0.9, 0.9);
                leg->AddEntry(g_plot, "Target: b1ch1", "l");
                leg->AddEntry(g_trigger, "Trigger: b4ch9", "l");
                leg->Draw();

                c->SaveAs((std::string("test2_b1ch1_waveforms_b4ch9_triggered/event_") + std::to_string(event_id) + "_from_" + filename_suffix + ".png").c_str());
                delete c;
                delete leg;
                delete mg;
            }
...
void plot_b1ch1_triggered_on_b4ch9(const std::vector<std::string>& filenames) {
    gSystem->mkdir("b1ch1_waveforms_b4ch9_triggered", kTRUE);
    int total_triggered_events = 0;
    for (const auto& filename : filenames) {
        TFile *f = TFile::Open(filename.c_str());
        ...
        // Extract filename suffix for plot naming
        std::regex re(".*majority_test_251031T0706_(\\d+).root");
        std::smatch match;
        std::string filename_suffix = "unknown";
        if (std::regex_search(filename, match, re) && match.size() > 1) {
            filename_suffix = match.str(1);
...
void plot_b4ch9_triggered_on_b1ch1(const std::vector<std::string>& filenames) {
    gSystem->mkdir("b4ch9_waveforms_b1ch1_triggered", kTRUE);
    int total_triggered_events = 0;
    for (const auto& filename : filenames) {
        TFile *f = TFile::Open(filename.c_str());
        if (!f || f->IsZombie()) continue;
        ...
        unsigned int event_id;
        const char* trigger_branch_name = "adc_b1_ch1";
        const char* plot_branch_name = "adc_b4_ch9";
        t->SetBranchAddress(trigger_branch_name, trigger_waveform);
        t->SetBranchAddress(plot_branch_name, plot_waveform);
        t->SetBranchAddress("event_id", &event_id);
        ...
        for (int i = 0; i < 20; i++) {
            ...
            for (int j = 0; j < 1920; j++) {
                if (-(trigger_waveform[j] - trigger_baseline) * ADC_TO_MV_B1 > 15) {
                    trigger = true;
                    break;
                }
            }
            ...
            if (trigger) {
                ...
                TGraph *g_plot = new TGraph(240);
                for(int j=0; j<240; j++) g_plot->SetPoint(j, j * B4_TIME_CONVERSION, -(plot_waveform[j] - plot_baseline) * ADC_TO_MV_B4);
                g_plot->SetLineColor(kBlue);
                g_plot->SetLineWidth(2);
                mg->Add(g_plot);

                TGraph *g_trigger = new TGraph(1920);
                for(int j=0; j<1920; j++) g_trigger->SetPoint(j, j * B1_TIME_CONVERSION, -(trigger_waveform[j] - trigger_baseline) * ADC_TO_MV_B1);
                g_trigger->SetLineColor(kRed);
                g_trigger->SetLineWidth(2);
                mg->Add(g_trigger);
                ...
                TLegend *leg = new TLegend(0.7, 0.8, 0.9, 0.9);
                leg->AddEntry(g_plot, "Target: b4ch9", "l");
                leg->AddEntry(g_trigger, "Trigger: b1ch1", "l");
                leg->Draw();

                c->SaveAs((std::string("test2_b4ch9_waveforms_b1ch1_triggered/event_") + std::to_string(event_id) + "_from_" + filename_suffix + ".png").c_str());
                delete c;
                delete leg;
                delete mg;
            }
    ...
    std::cout << "plot_b4ch9_triggered_on_b1ch1: Saved " << total_triggered_events << " plots." << std::endl;
}


void plot_b1ch1_triggered_on_b1ch1_bu(const std::vector<std::string>& filenames) {
    gSystem->mkdir("b1ch1_waveforms_b1ch1_triggered", kTRUE);
    int total_triggered_events = 0;
    for (const auto& filename : filenames) {
        TFile *f = TFile::Open(filename.c_str());
        if (!f || f->IsZombie()) continue;
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (trigger) {
                ...
                c->SaveAs((std::string("test2_b1ch1_waveforms_b1ch1_triggered/event_") + std::to_string(event_id) + "_from_" + filename_suffix + ".png").c_str());
                delete c;
                delete leg;
                delete mg;
            }
...
void plot_b1ch1_triggered_on_b1ch1(const std::vector<std::string>& filenames) {
    gSystem->mkdir("b1ch1_waveforms_b1ch1_triggered", kTRUE);
    int total_triggered_events = 0;
    for (const auto& filename : filenames) {
        TFile *f = TFile::Open(filename.c_str());
...
void plot_integrated_sum(const std::vector<std::string>& filenames, bool trigger_on_b4ch9) {
    gSystem->mkdir("integrated_sum_histograms", kTRUE);
...
void analysis2() {
	...
	"data_30ton/majority_test_251031T0706_0.root",
	"data_30ton/majority_test_251031T0706_1.root",
/*	    
...
*/
    ...
    plot_b4ch9_triggered_on_b1ch1(task2_and_integrated_sum_filenames);
