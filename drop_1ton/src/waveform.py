...
class Waveform():
        ...
        if self.cfg.daisy_chain:
            for ch in self.ch_names:
                if '_b1' in ch:
                    ...
                    return None
                if '_b5' in ch:
                    n_samp_b5 = len(self.amp_mV[ch])
                    pre_trg_frac = 1.0-self.cfg.post_trigger
                    self.trg_pos = int(n_samp_b5*pre_trg_frac)
                    self.trg_time_ns = self.trg_pos*(SAMPLE_TO_NS)
        else:
    ...
    def subtract_flat_baseline(self):
        ...
        adc_to_mV_b5 = self.cfg.dgtz_dynamic_range_mV/(2**12-1)
        for ch in self.ch_names:
            val = self.raw_data[ch]
            if 'adc_b5_ch0' in ch and self.cfg.debug:
            ...
            if 'b5' in ch:
                self.amp_mV[ch] = amp*adc_to_mV_b5
            else:
    ...
    def define_time_axis(self):
        ...
        n_samp_b5 = len(self.amp_mV['adc_b5_ch0'])
        #n_samp_b5 = 1
        t3=np.linspace(0, (n_samp_b5-1)*SAMPLE_TO_NS, n_samp_b5)
        self.time_axis_ns_b5 = t3
        self.n_samp_b5 = n_samp_b5

        #if self.cfg.use_hodoscope:
        #    self.n_samp_b5 = len(self.amp_pe['adc_b5_ch1'])
        #else:
        #    self.n_samp_b5 = 0
